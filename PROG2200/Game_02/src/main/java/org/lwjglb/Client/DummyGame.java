...
//    }
    ...
    public void createObjects() {
        lengthOfList = client.getObject().size();
        scene.removeAll();
        scene.setGameItems(new GameItem[] {terrain});
        if (client.getObject() != null) {
            for (ObjDbConverter obj : client.getObject()) {
                if (obj.getVeh_type() != null) {
                    if (obj.getVeh_type().equals("LandVehicle")) {
                        ...
                        house.setIdentity(obj.getIdentity());
...
//        }
    ...
    public void saveGame(List<GameItem> gameItems) {
//        em.getTransaction().begin();
//
...
//
//        for (GameItem items: gameItems) {
//            ObjDbConverter obj = new ObjDbConverter(items.getPosition().x, items.getPosition().y, items.getPosition().z, items.getThisIs());
//            em.persist(obj);
//        }
...
//        server.sendToClient(load);
        ...
        if(lengthOfList < client.getObject().size()) {
            lengthOfList = client.getObject().size();
            sceneChanged = true;
            removeAll = true;
            createObjects();
        ...
        for (GameItem item : gameItems) {
            if (item.getThisIs() != null) {
                for (ObjDbConverter obj : client.getObject()) {
                    if (obj.getIdentity()== item.getIdentity()) {
                        item.setPosition(obj.getX(), obj.getY(), obj.getZ());
                    }
                }
            }
        }
