...
public class Transformation {
    ...
    /**
     * Russ <RS>  Changed this code back to an older
     * version where rotation worked properly.
     * It seems the use of the 4th value of the
     * Quaternionf was not properly implemented
     * @param gameItem
     * @return The matrix that represents the new position and rotation of the gameitem.
     */
    public Matrix4f buildModelMatrix(GameItem gameItem) {

        // <RS> old old code that rotated properly
        Quaternionf rotation = gameItem.getRotation();
        return modelViewMatrix.identity().translate(gameItem.getPosition()).
